...
function Categories() {
  ...
  const { data, isLoading, error, makeRequest } = useCommonApi<Category>('/api/v1/category');
                  ...
                  <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(onSubmit)}>
                      <div>
                          ...
                          className="bg-gray-50 
                            ...
                            {error && <p className='text-red-500 text-sm mt-1' role="alert">This category is already exists!</p>}
                        
                      </div>
                    
                      <button type="submit" className="max-w-md text-center inline-block shrink-0 rounded-md border border-blue-600 bg-blue-600 px-12 py-3 text-sm font-medium text-white transition hover:bg-transparent hover:text-blue-600 focus:outline-none focus:ring active:text-blue-500">{ isMode > 0 ? 'EDIT' : 'ADD' }</button>
                  ...
                  { data && data?.map ((res, index) => (
                      ...
                      <td className="py-3 px-6 text-center">
                        <div className="flex item-center justify-center">
                          ...
                          <div className="w-5 mr-3 transform hover:text-purple-500 hover:scale-110" onClick={ () => editHandle(res._id, res.category)}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                            </svg>
                          </div>
                          <div className="w-5 mr-3 transform hover:text-purple-500 hover:scale-110" onClick={ () => delHandle(res._id) }>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
